...
trait Symbols extends api.Symbols { self: SymbolTable =>
  ...
  abstract class Symbol protected[Symbols] (initOwner: Symbol, initPos: Position, initName: Name)
    ...
    private[this] def _validTo: Period = __validTo
    private[this] def _validTo_= (p: Period) = {

      if (this.name.toString.startsWith("F$mc") && this.name.toString.endsWith("$sp")) {
        println("\n\n\nThis is " + this + ": I got my validTo changed!!!")
        println("prev validTo: " + (if (__validTo != NoPeriod) phaseOf(__validTo) else "NoPeriod"))
        println("new validTo:  " + (if (p != NoPeriod) phaseOf(p) else "NoPeriod"))
        println("phase:        " + phase)
        if (p == NoPeriod) {
          println("stack:        ")
...
*/
    ...
    def info: Type = try {
      ...
      while (validTo == NoPeriod) {
        ...
        try {
          ...
          if (this.name.toString.startsWith("F$mc") && this.name.toString.endsWith("$sp")) {
            println("\n\n\nThis is " + this + ": I'm getting completed")
            println("validTo:      " + (if (validTo != NoPeriod) phaseOf(validTo) else "NoPeriod"))
            println("stack:        ")
            println(Thread.currentThread.getStackTrace.mkString("    at ", "\n    at ", "\n") + "\n\n\n")
          }
          tp.complete(this)
    ...
    def info_=(info: Type) {
      ...
      if (this.name.toString.startsWith("F$mc") && this.name.toString.endsWith("$sp")) {
        println("\n\n\nThis is " + this + ": I got my info changed!")
        println("prev history: " + (if (infos ne null) this.infosString else "<null>"))
        infos = TypeHistory(currentPeriod, info, null)
        println("new history:      " + this.infosString)
        println("phase:        " + phase)
        println("stack:        ")
        println(Thread.currentThread.getStackTrace.mkString("    at ", "\n    at ", "\n") + "\n\n\n")
      } else
