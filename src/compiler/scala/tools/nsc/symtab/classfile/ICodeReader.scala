...
abstract class ICodeReader extends ClassfileParser {
  ...
  override def classNameToSymbol(name: Name) = {
    ...
    val sym = if (name.toString == fulltpnme.RuntimeNothing.toString) {
      ...
      val result = enteringFlatten { iface.implClass } // why enteringFlatten? explained in method postTransform in Mixins.scala
      ...
      val result =
        try {
          exitingFlatten(rootMirror.staticClass(name.toTypeName.toString))
        } catch {
...
//      method.dump
    ...
    def resolveNEWs() {
      ...
      for (bb <- method.code.blocks ; (i, idx) <- bb.toList.zipWithIndex) i match {
        case cm @ CALL_METHOD(m, Static(true), _) if m.isClassConstructor =>
