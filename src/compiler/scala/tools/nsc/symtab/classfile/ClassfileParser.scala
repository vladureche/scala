  ...
  class ConstantPool {
    ...
    def getMemberSymbol(index: Int, static: Boolean): Symbol = {
      ...
      if (f eq null) {
        ...
        //forceMangledName(tpe0.resultType.typeSymbol.name, false)
...
//            println("Looking for: " + name + ": " + tpe + " inside: " + ownerTpe.typeSymbol + "\n\tand found: " + ownerTpe.members)
  ...
  def forceMangledName(name: Name, module: Boolean): Symbol = {
    ...
    // start the search
    val result = beforeFlatten { searchDeep(parts, sym, 0, module) }
  ...
  def searchDeep(parts: Array[String], sym: Symbol, index: Int, module: Boolean): Symbol = {
    // backtracking search for symbol - like scaladoc does for links :)
    def search(term: Boolean): Symbol = {
      searchDeep(parts, {
        import scala.reflect.NameTransformer._
        sym.linkedClassOfClass.info
        val name = if (term) newTermName(decode(parts(index))) else newTypeName(decode(parts(index)))
        debuglog("forceMangledName: looking for " + name + " (term=" + name.isTermName + ") in " + sym)
        val result = sym.info.decls.lookup(name)
        if (result == NoSymbol)
          debuglog(name + " in:\n  " + sym.info.decls.filter(_.name.startsWith(name.toString.apply(0))))
        result
      }, index + 1, module)
    }
    ...
    if (sym == NoSymbol)
      NoSymbol
    else if (sym.alternatives.length != 1) {
      debuglog("forceMangledName: visiting alternatives: " + sym.alternatives)
      sym.alternatives.foldLeft(NoSymbol: Symbol)((res, s) => if (res == NoSymbol) searchDeep(parts, s, index, module) else res)
    } else if (index < parts.length - 1) {
      var sym1 = search(term = true)
      debuglog("forceMangledName: found NoSymbol for term search. Backtracking")
      if (sym1 == NoSymbol)
        sym1 = search(term = false)
      if (sym1 == NoSymbol)
        NoSymbol
      else
        sym1
...
//        println("\tadding " + (t.typeSymbol.name + "->" + t.typeSymbol))
  ...
  private def sigToType(sym: Symbol, sig: Name): Type = {
    ...
    def sig2type(tparams: immutable.Map[Name,Symbol], skiptvs: Boolean): Type = {
      ...
      // This is too aggresive unfortunately, and it prevents legitimate things from being inlined
      // TODO: RE-ENABLE THIS CHECK ASAP and FIX what it complains about!
      if (tpe == NoType)
        throw new ICodeReaderException("NoType generated by sig2type for: " + sig)
      tpe
    } // sig2type(tparams, skiptvs)
