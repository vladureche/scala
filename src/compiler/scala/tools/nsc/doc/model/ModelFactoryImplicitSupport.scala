...
trait ModelFactoryImplicitSupport {
  ...
  def makeImplicitConversions(sym: Symbol, inTpl: => DocTemplateImpl): List[ImplicitConversion] =
    if (!(sym.isClass || sym.isTrait)) Nil
      ...
      val conversions = results.flatMap(result => makeImplicitConversion(sym, result._1, result._2, context, inTpl))
  ...
  def makeImplicitConstraints(types: List[Type], sym: Symbol, context: Context, inTpl: => DocTemplateImpl): List[Constraint] = 
    types.flatMap((tpe:Type) => {
      ...
      if (eliminateImplicits && typeVarsInType(tpe).isEmpty) {
        try {
          if (hardcoded.autoGenerated.contains(qualifiedName))
            available = Some(true)
          else {
            val search = new ImplicitSearch(EmptyTree, implType, false, context.makeImplicit(false)).allImplicitsPoly(Nil)
            context.flushBuffer() /* any errors here should not prevent future findings */
            available = Some(!search.isEmpty)            
          }
        } catch {
          case _ =>
        }
      ...
      available match {
        case Some(true) => 
          Nil
        case Some(false) => 
          error(qualifiedName + "  " + hardcoded.autoGenerated.contains(qualifiedName))
...
object hardcoded {
  ...
  /** Classes for which the type class is automatically generated, should avoid the implicit search */
  val autoGenerated: List[String] = List("<root>.scala.reflect.Manifest", "<root>.scala.reflect.ClassManifest", "<root>.scala.reflect.OptManifest")
}